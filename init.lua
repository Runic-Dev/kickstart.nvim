...
require('lazy').setup({
  ...
  },
  {
    ...
    opts = {
      edit_commands = { -- Allow CopilotChat to apply changes directly
        inline_edit = true, -- Allow inline edits
        buffer_edit = true, -- Allow modifications in buffers
      },
      window = {
        layout = 'vertical', -- Make the chat window vertical for better editing experience
        width = 0.4, -- Adjust width of CopilotChat window
      },
      mappings = {
        accept_edit = '<C-y>', -- Apply edits from CopilotChat
        discard_edit = '<C-x>', -- Discard suggested edits
      },
    },
    keys = {
      -- Toggle CopilotChat
      { '<leader>cc', '<cmd>CopilotChatToggle<CR>', desc = 'Toggle CopilotChat' },
      -- Ask CopilotChat to refactor the selected code
      { '<leader>cr', ':CopilotChatRefactor<CR>', desc = 'Refactor Code with CopilotChat', mode = { 'v', 'n' } },
      -- Apply Copilot edits
      { '<leader>ca', ':CopilotChatApply<CR>', desc = 'Apply CopilotChat Edit' },
    },
  },
}, {
  ui = {
